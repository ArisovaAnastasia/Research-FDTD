...
int main(int argc, char* argv[]) {
    ...
    constexpr int delta_x = 8, delta_y = 0, delta_z = 0;
    ftype_sigma sigma_x = 46.5, sigma_y = 46.5;
    constexpr int Nx = 256, Ny = 1, Nz = 1;
    ...
    try {
        ...
        */
        ...
        ofstream numbEy("All split field on start PML.csv");
        ofstream numb("Energy.csv");
...
*/
            Calculate_Currents_sycl_half_version<ftype>(cube, dy, dz, cd, fg, offset, index_start_x, index_start_y, index_start_z, 0, 0, A,
                t1, t2, t0_x, tp_x, y00, x1, x2, ay_transverse, omega_, k_);
           ...
           /* numbEy << it << ";"<< cube.Ex(8 + Nx + 1, 1, 1) << ";" << cube.Ey(8 + Nx + 1, 1, 1) << ";" << cube.Ez(8 + Nx + 1, 1, 1)
                << ";" << cube.Bx(8 + Nx + 1, 1, 1) << ";" << cube.By(8 + Nx + 1, 1, 1) << ";" << cube.Bz(8 + Nx + 1, 1, 1) << std::endl;*/


            numbEy << it << ";"<< cube_split.Exy(8 + Nx + 1, 1, 1) << ";" << cube_split.Exz(8 + Nx + 1, 1, 1) << ";" << cube_split.Eyx(8 + Nx + 1, 1, 1)
                   << ";" << cube_split.Eyz(8 + Nx + 1, 1, 1) << ";" << cube_split.Ezx(8 + Nx + 1, 1, 1) << ";" << cube_split.Ezy(8 + Nx + 1, 1, 1) << ";";
            numbEy << cube_split.Bxy(8 + Nx + 1, 1, 1) << ";" << cube_split.Bxz(8 + Nx + 1, 1, 1) << ";" << cube_split.Byx(8 + Nx + 1, 1, 1)
                << ";" << cube_split.Byz(8 + Nx + 1, 1, 1) << ";" << cube_split.Bzx(8 + Nx + 1, 1, 1) << ";" << cube_split.Bzy(8 + Nx + 1, 1, 1) << std::endl;
            ...
            Update_PMLBound_electric(cube, cube_split, Coeff, Nx, Ny, Nz,
                    delta_x, delta_y, delta_z, _1dx, _1dy, _1dz);

            Update_PML_electric<ftype, ftypePML>(cube_split, Coeff,
            ...
            // 
            // 
            //edge from 0,0,0 by YX DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(Nx, Ny, THREAD_COUNT_SPEC), [=](auto index) {
                    int i = index[0] + delta_x + 1;
                    int j = index[1] + delta_y + 1;
                    int k = index[2] + 1;
            ...
            //
            //Обработка четырех областей "колонн"
            //
                
            //column from 0,0,0 DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT, THREAD_COUNT_SPEC), [=](auto index) {
                    for (int i = index[0] + 1; i < delta_x + 1; i += THREAD_COUNT)
                        for (int j = index[1] + 1; j < delta_y + 1; j += THREAD_COUNT)
                            for (int k = index[2] + 1; k < Nz + 2 * delta_z + 1; k += THREAD_COUNT_SPEC) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
                }).wait_and_throw();

            //column from n,0,0 DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT_SPEC, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + Nx + delta_x + 1; i < Nx + 2 * delta_x + 1; i += THREAD_COUNT)
                        for (int j = index[1] + 1; j < delta_y + 1; j += THREAD_COUNT_SPEC)
                            for (int k = index[2] + 1; k < Nz + 2 * delta_z + 1; k += THREAD_COUNT) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
                }).wait_and_throw();

            //column from 0,m,0 DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT_SPEC, THREAD_COUNT, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + 1; i < delta_x + 1; i += THREAD_COUNT)
                        for (int j = index[1] + Ny + delta_y + 1; j < Ny + 2 * delta_y + 1; j += THREAD_COUNT)
                            for (int k = index[2] + 1; k < Nz + 2 * delta_z + 1; k += THREAD_COUNT) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
            ...
            //
            // Обработка областей под крыльями
            // 
            
            //area under the wings from 0,0,0 by YZ DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT, THREAD_COUNT_SPEC), [=](auto index) {
                    for (int i = index[0] + 1; i < delta_x + 1; i += THREAD_COUNT)
                        for (int j = index[1] + delta_y + 1; j < Ny + delta_y + 1; j += THREAD_COUNT)
                            for (int k = index[2] + delta_z + 1; k < Nz + delta_z + 1; k += THREAD_COUNT_SPEC) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
                }).wait_and_throw();

            //area under the wings from n,0,0 by YZ DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT_SPEC, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + Nx + delta_x + 1; i < Nx + 2 * delta_x + 1; i += THREAD_COUNT)
                        for (int j = index[1] + delta_y + 1; j < Ny + delta_y + 1; j += THREAD_COUNT_SPEC)
                            for (int k = index[2] + delta_z + 1; k < Nz + delta_z + 1; k += THREAD_COUNT) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
            ...
            //area under the wings from 0,0,0 by XZ DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT_SPEC, THREAD_COUNT, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + delta_x + 1; i < Nx + delta_x + 1; i += THREAD_COUNT_SPEC)
                        for (int j = index[1] + 1; j < delta_y + 1; j += THREAD_COUNT)
                            for (int k = index[2] + 1; k < delta_z + 1; k += THREAD_COUNT) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
                }).wait_and_throw();

            //area under the wings from 0,m,0 by XZ DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT_SPEC, THREAD_COUNT, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + delta_x + 1; i < Nx + delta_x + 1; i += THREAD_COUNT_SPEC)
                        for (int j = index[1] + Ny + delta_y + 1; j < Ny + 2 * delta_y + 1; j += THREAD_COUNT)
            ...
            //
            // Обработка областей над крыльями
            // 
            

            //area over the wings from 0,0,0 by YZ DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT_SPEC, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + 1; i < delta_x + 1; i += THREAD_COUNT)
                        for (int j = index[1] + delta_y + 1; j < Ny + delta_y + 1; j += THREAD_COUNT_SPEC)
                            for (int k = index[2] + Nz + delta_z + 1; k < Nz + 2 * delta_z + 1; k += THREAD_COUNT) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
                }).wait_and_throw();

            //area over the wings from n,0,0 by YZ DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT_SPEC, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + Nx + delta_x + 1; i < Nx + 2 * delta_x + 1; i += THREAD_COUNT)
                        for (int j = index[1] + delta_y + 1; j < Ny + delta_y + 1; j += THREAD_COUNT_SPEC)
            ...
            //area over the wings from 0,m,0 by YZ DONE
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT, THREAD_COUNT_SPEC), [=](auto index) {
                    for (int i = index[0] + delta_x + 1; i < Nx + delta_x + 1; i += THREAD_COUNT)
                        for (int j = index[1] + Ny + delta_y + 1; j < Ny + 2 * delta_y + 1; j += THREAD_COUNT)
            ...
            */
            ...
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT_SPEC, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + 1; i < Nx + 2 * delta_x + 1; i += THREAD_COUNT)
                        for (int j = index[1] + 1; j < delta_y; j += THREAD_COUNT_SPEC)
                            for (int k = index[2] + delta_z; k < Nz + 2 * delta_z + 1; k += THREAD_COUNT) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
                }).wait_and_throw();
            ...
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT_SPEC, THREAD_COUNT, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + 1; i < delta_x; i += THREAD_COUNT_SPEC)
                        for (int j = index[1] + delta_y; j < Ny + 2 * delta_y + 1; j += THREAD_COUNT)
                            for (int k = index[2] + delta_z; k < Nz + 2 * delta_z + 1; k += THREAD_COUNT) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
            ...
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT_SPEC, THREAD_COUNT, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + Nx + delta_x + 2; i < Nx + 2 * delta_x + 1; i += THREAD_COUNT_SPEC)
                        for (int j = index[1] + delta_y; j < Ny + 2 * delta_y + 1; j += THREAD_COUNT)
                            for (int k = index[2] + delta_z; k < Nz + 2 * delta_z + 1; k += THREAD_COUNT) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
            ...
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT_SPEC, THREAD_COUNT), [=](auto index) {
                    for (int i = index[0] + delta_x; i < Nx + delta_x + 2; i += THREAD_COUNT)
                        for (int j = index[1] + Ny + delta_y + 2; j < Ny + 2 * delta_y + 1; j += THREAD_COUNT_SPEC)
                            for (int k = index[2] + delta_z; k < Nz + 2 * delta_z + 1; k += THREAD_COUNT) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
            ...
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT, THREAD_COUNT_SPEC), [=](auto index) {
                    for (int i = index[0] + delta_x; i < Nx + delta_x + 2; i += THREAD_COUNT)
                        for (int j = index[1] + delta_y; j < Ny + delta_y + 2; j += THREAD_COUNT)
                            for (int k = index[2] + Nz + delta_z + 2; k < Nz + 2 * delta_z + 1; k += THREAD_COUNT_SPEC) {
                                Update_electric_field_PML_sycl_half_version<ftype, ftypePML>(cube_split, Coeff, _1dx, _1dy, _1dz, i, j, k);
                            }
                    });
                }).wait_and_throw();
            */
            Run_CubeSplit2Cube_electric(cube, cube_split, Nx, Ny, Nz, delta_x, delta_y, delta_z);

            //Update_PeriodicBound_electric<ftype, ftypePML>(cube, Nx, Ny, Nz, delta_x, delta_y, delta_z);
            ...
            Update_PMLBound_magnetic(cube, cube_split, Coeff, Nx, Ny, Nz,
                    delta_x, delta_y, delta_z, _1dx, _1dy, _1dz);
            Update_PML_magnetic<ftype, ftypePML>(cube_split, Coeff,
                _1dx, _1dy, _1dz, Nx, Ny, Nz, delta_x, delta_y, delta_z);
            ...
            */
            ...
            /*
            //std::cout << "Success Update_PMLBound_magnetic" << std::endl;
            //edge from 0,0,0 by XY
            q.submit([&](sycl::handler& h) {
                sycl::stream out(4096, 4096, h);
                h.parallel_for(range<3>(THREAD_COUNT, THREAD_COUNT, THREAD_COUNT_SPEC), [=](auto index) {
                ...
                }).wait_and_throw(); //std::cout << "Success magnetic xy" << std::endl;
            ...
            q.submit([&](sycl::handler& h) {
                ...
                }).wait_and_throw(); //std::cout << "Success magnetic xz" << std::endl;
            ...
            q.submit([&](sycl::handler& h) {
                ...
                }).wait_and_throw(); //std::cout << "Success magnetic yz" << std::endl;
...
*/
            Run_CubeSplit2Cube_magnetic(cube, cube_split, Nx, Ny, Nz, delta_x, delta_y, delta_z);
